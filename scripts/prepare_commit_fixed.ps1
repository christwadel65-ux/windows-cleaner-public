...
Usage (PowerShell):
.\scripts\prepare_commit_fixed.ps1

What it does:
- Runs `dotnet format` if available (or suggests installation).
- Runs `dotnet build` to ensure the project builds.
- Optionally stages all changes and creates a single commit with a standard message.
