...
namespace WindowsCleaner
{
    public static class LanguageManager
    {
        ...
        private static Dictionary<string, Dictionary<Language, string>> _translations = new Dictionary<string, Dictionary<Language, string>>
        {
            ...
            { "uninstaller_publisher_header", new Dictionary<Language, string> { { Language.French, "Éditeur" }, { Language.English, "Publisher" } } },
            { "uninstaller_size_header", new Dictionary<Language, string> { { Language.French, "Taille (MB)" }, { Language.English, "Size (MB)" } } },
            { "uninstaller_cleanup_remnants", new Dictionary<Language, string> { { Language.French, "Nettoyer les fichiers résiduels après désinstallation" }, { Language.English, "Clean residual files after uninstallation" } } },
            { "btn_uninstall", new Dictionary<Language, string> { { Language.French, "Désinstaller" }, { Language.English, "Uninstall" } } },
            { "btn_export_csv", new Dictionary<Language, string> { { Language.French, "Exporter en CSV" }, { Language.English, "Export to CSV" } } },
            { "uninstaller_no_selection", new Dictionary<Language, string> { { Language.French, "Veuillez sélectionner au moins un programme" }, { Language.English, "Please select at least one program" } } },
            { "uninstaller_confirm", new Dictionary<Language, string> { { Language.French, "Êtes-vous sûr de vouloir désinstaller {0} programme(s) ?\n\nCette action ne peut pas être annulée." }, { Language.English, "Are you sure you want to uninstall {0} program(s)?\n\nThis action cannot be undone." } } },
            { "uninstaller_programs_found", new Dictionary<Language, string> { { Language.French, "{0} programmes trouvés" }, { Language.English, "{0} programs found" } } },
            { "uninstaller_uninstalling", new Dictionary<Language, string> { { Language.French, "--- Désinstallation de {0} ---" }, { Language.English, "--- Uninstalling {0} ---" } } },
            { "uninstaller_cleanup", new Dictionary<Language, string> { { Language.French, "Nettoyage des restes..." }, { Language.English, "Cleaning remnants..." } } },
            { "uninstaller_success", new Dictionary<Language, string> { { Language.French, "✓ Désinstallations réussies : {0}" }, { Language.English, "✓ Successful uninstalls: {0}" } } },
            { "uninstaller_failed", new Dictionary<Language, string> { { Language.French, "✗ Désinstallations échouées : {0}" }, { Language.English, "✗ Failed uninstalls: {0}" } } },
            { "uninstaller_loading", new Dictionary<Language, string> { { Language.French, "Chargement des programmes installés..." }, { Language.English, "Loading installed programs..." } } },
            { "uninstaller_export_success", new Dictionary<Language, string> { { Language.French, "Liste exportée vers :\n{0}" }, { Language.English, "List exported to:\n{0}" } } },
            { "uninstaller_export_error", new Dictionary<Language, string> { { Language.French, "Erreur lors de l'export" }, { Language.English, "Export error" } } },
            { "uninstaller_attention", new Dictionary<Language, string> { { Language.French, "Attention" }, { Language.English, "Warning" } } },
