...
namespace WindowsCleaner
{
    ...
    public static class SystemOptimizer
    {
        ...
        public static bool OptimizeSsd(Action<string>? log = null)
        {
            try
            {
                log?.Invoke(LanguageManager.Get("sysopt_trim_running"));
                ...
                if (process == null)
                {
                    log?.Invoke(LanguageManager.Get("sysopt_cannot_start_defrag"));
                    return false;
                }
                    ...
                    if (drive.IsReady && drive.DriveType == DriveType.Fixed)
                    {
                        try
                        {
                            ...
                            if (trimProcess != null)
                            {
                                ...
                                log?.Invoke(LanguageManager.Get("sysopt_trim_executed", drive.Name));
                            }
                        }
                        catch (Exception ex)
                        {
                            log?.Invoke(LanguageManager.Get("sysopt_trim_error", drive.Name, ex.Message));
                        }
                    }
                }
                
                log?.Invoke(LanguageManager.Get("sysopt_trim_finished"));
                Logger.Log(LogLevel.Info, LanguageManager.Get("sysopt_trim_completed"));
                return true;
            }
            catch (Exception ex)
            {
                log?.Invoke(LanguageManager.Get("error_cleaning", "optimisation SSD", ex.Message));
        ...
        public static bool CompactRegistry(Action<string>? log = null)
        {
            try
            {
                log?.Invoke(LanguageManager.Get("sysopt_registry_compacting"));
                ...
                var script = @"
                    ...
                    Write-Output '" + LanguageManager.Get("sysopt_registry_scheduled") + @"'
                ";
                ...
                log?.Invoke(LanguageManager.Get("sysopt_registry_scheduled"));
                Logger.Log(LogLevel.Info, LanguageManager.Get("sysopt_registry_completed"));
                return true;
            }
            catch (Exception ex)
            {
                log?.Invoke(LanguageManager.Get("error_cleaning", "compactage registre", ex.Message));
                Logger.Log(LogLevel.Error, LanguageManager.Get("error_cleaning", "compactage registre", ex.Message));
                return false;
            }
        }
        ...
        public static bool ClearStandbyMemory(Action<string>? log = null)
        {
            try
            {
                log?.Invoke(LanguageManager.Get("sysopt_memory_clearing"));
...
'@
                    ...
                    [System.GC]::Collect()
                    Write-Output '" + LanguageManager.Get("sysopt_memory_cleared") + @"'
                ";
                ...
                if (process == null)
                {
                    ...
                    log?.Invoke(LanguageManager.Get("sysopt_memory_cleared"));
                    return true;
                }
                ...
                log?.Invoke(LanguageManager.Get("sysopt_memory_cleared"));
                Logger.Log(LogLevel.Info, LanguageManager.Get("sysopt_memory_cleared"));
                return true;
            }
            catch (Exception ex)
            {
                log?.Invoke(LanguageManager.Get("error_cleaning", "cache mémoire", ex.Message));
